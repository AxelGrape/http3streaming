...
#include <proxygen/httpserver/samples/laget/HQServer.h>
...
#include <proxygen/httpserver/samples/laget/FizzContext.h>
#include <proxygen/httpserver/samples/laget/HQLoggerHelper.h>
#include <proxygen/httpserver/samples/laget/HQParams.h>
#include <proxygen/httpserver/samples/laget/SampleHandlers.h>
...
HTTPTransactionHandler* Dispatcher::getRequestHandler(HTTPMessage* msg,
  ...
  if (!params.staticRoot.empty()) {
    if (path == "/getnames"){
      std::string root_ = folly::to<std::string>(params.staticRoot);
      std::ofstream file(folly::to<std::string>(root_,"/", path));
      for (const auto & entry : fs::directory_iterator(root_))
          file << "/" << entry.path().string().substr(root_.length()) << std::endl;
    }
